...
-- Dump completed
 ...
 select bid.bidder_id, bd.bidder_name, count(bid.bidder_id),sum(if(bid.bid_status = 'Won',1,0)), sum(if(bid.bid_status = 'Won',1,0)),
(sum(if(bid.bid_status = 'Won',1,0))/ count(bid.bidder_id)*100) as bidder_winning_per from ipl_bidding_details as bid
join ipl_bidder_details as bd
on bd.bidder_id = bid.bidder_id
group by bidder_name
order by (sum(if(bid.bid_status = 'Won',1,0))/ count(bid.bidder_id)*100) desc;

 
 -- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
  select i_s.stadium_name,i_s.city,count(stadium_name) from IPL_Stadium as i_s
 right join  IPL_match_schedule as ms
 on i_s .STADIUM_ID = ms.STADIUM_ID
 group by stadium_name 
 order by count(stadium_name);
 
 -- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select I_s.Stadium_name,count(im.match_id) as total_match,SUM(if(IM.toss_winner = IM.match_winner,1,0)) as number_of_matches_won_by_toss,
(SUM(if(IM.toss_winner = IM.match_winner,1,0))/count(im.match_id)*100) as percentage from Ipl_match_schedule IMC
join IPL_Stadium AS I_S
on IMC.Stadium_id = I_S.stadium_id
join IPL_match as IM
group by Stadium_name;


-- 4.	Show the total bids along with bid team and team name.
select  BD.BID_TEAM,count(bd.BID_TEAM),IT.Team_Name from IPL_BIDDing_details as BD
 JOIN IPL_Team AS IT
on BD. BID_Team = IT.Team_id
group by BID_TEAM;

-- 5.	Show the team id who won the match as per the win details.
select a.team_id,a.team_name,b.team_id1,b.team_id2,b.match_winner,b.win_details
from ipl_team as a
join ipl_match as b
on substr(a.remarks,1,3)= substr(b.win_details,6,3);


-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select its.team_id,it.team_name,sum(its.matches_played),sum(its.matches_won),sum(its.matches_lost)  from ipl_team_standings as ITS
join ipl_team as it
...
Select ip.Player_name,it.team_name,itp.player_role from ipl_team as it
join ipl_team_players as itp
...
Select it.team_name,itp.player_role,count(itp.player_role) from ipl_team as it
join ipl_team_players as itp
on it.team_id =  itp.team_id
join ipl_player as ip
on itp.player_id = ip.player_id
where  Player_role like '%All%'
group by team_name having count(itp.player_role) > 4;
